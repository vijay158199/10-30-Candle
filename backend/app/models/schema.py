...
class Trade(Base):
    ...
    # Nullable: a session can end before the swing ever breaks, in which
    # case direction is genuinely unknown, not just "not yet decided".
    direction: Mapped[str | None] = mapped_column(String(8), nullable=True)  # BUY | SELL

    # Unused by this strategy (no separate liquidity-tap stage - the swing
    # break itself is stage 1) - kept nullable/unset for schema parity with
    # the sibling projects rather than removed.
    ...
    liquidity_side: Mapped[str | None] = mapped_column(String(8), nullable=True)

    entry_time: Mapped[dt.datetime | None] = mapped_column(DateTime, nullable=True)
    entry_price: Mapped[float | None] = mapped_column(Float, nullable=True)
    entry_type: Mapped[str | None] = mapped_column(String(24), nullable=True)  # always NEXT_CANDLE_OPEN for this strategy
    ...
    mss_choch_bos: Mapped[str | None] = mapped_column(String(16), nullable=True)  # always SWING_BREAK for this strategy
    structure_time: Mapped[dt.datetime | None] = mapped_column(DateTime, nullable=True)  # when the swing actually broke
    explosive_candle_count: Mapped[int | None] = mapped_column(Integer, nullable=True)  # unused - no multi-candle leg concept here
    # Human-readable reason(s) the pipeline stopped where it did (e.g. "No
    # 5m candle closed beyond the tracked swing high/low with real
    # displacement...") - engine.run_day always computes this (TradeResult.
    # notes), persisted here for the dashboard.
