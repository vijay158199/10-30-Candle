...
No timeframe selector here (unlike the sibling projects) - this strategy is
fixed to 5-minute candles per explicit spec, nothing to choose.
"""
...
def _get_or_create(session) -> LiveControl:
    ...
    if row is None:
        row = LiveControl(id=_ROW_ID, enabled=False)
        session.add(row)
        session.flush()
    return row
