...
def run_backtest(
    ...
    if existing_run_id is not None:
        ...
        with get_session() as session:
            run = BacktestRun(start_date=start_date, end_date=end_date, status="RUNNING", structure_interval="5m")
            session.add(run)
            session.flush()
            run_id = run.id
    ...
    try:
        ...
        for i, day in enumerate(days, start=1):
            try:
                sd_primary = get_session_data(symbol, day, structure_interval="5m")
