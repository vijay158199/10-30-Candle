...
_ACCENT = "#d9a548"      # --accent (dark) - brand gold, used for branding + structure marker
...
_SWING = "#8a93a3"       # pre-cutoff swing level (dotted)

_STRUCTURE_COLOR = _ACCENT
_ENTRY_COLORS = {
    "CISD": "#ec4899",
    "ORDER_BLOCK": "#3b82f6",
...
def _annotate_header(fig, ax, result: TradeResult, subtitle: str) -> None:
    ...
    fig.text(0.012, 0.965, "10:30 CANDLE STRATEGY", fontsize=8.5, fontweight="bold",
    ...
    fig.text(0.988, 0.015, "10:30 Candle · nifty-strategy dashboard", fontsize=6.5,
              color=_TEXT_MUTED, ha="right", va="bottom", alpha=0.65, family="sans-serif")


def _annotate_levels(fig, ax, result: TradeResult) -> None:
    """Draws the SWING/TP/ENTRY/SL horizontal lines at their true price on
    the chart, plus one compact caption line in the header band spelling
    out their values - a tight 10/20pt SL/TP on a ~24,000 NIFTY chart
    routinely puts those levels within a few pixels of each other, so
    price-anchored labels would overlap; a fixed caption in the header
    (outside the candle area entirely) is always readable regardless of how
    close the levels sit."""
    ...
    caption = "    ".join(f"{label} {price:,.1f}" for label, price, _ in items)
    fig.text(0.012, 0.895, caption, fontsize=8.5, fontweight="600",
...
def _annotate_structure(ax, y_top: float, y_span: float, plot_candles: pd.DataFrame, result: TradeResult) -> None:
    ...
    if pos is None:
        return
    ax.axvline(x=pos, color=_STRUCTURE_COLOR, linestyle=":", linewidth=1.3, zorder=1, ymax=0.95)
    ...
    ax.annotate(
        ...
        bbox=dict(boxstyle="round,pad=0.3", fc=_STRUCTURE_COLOR, ec="none"), zorder=5,
    )
...
def _annotate_entry_marker(ax, plot_candles: pd.DataFrame, result: TradeResult, y_span: float) -> None:
    ...
    # (below its low for a BUY, above its high for a SELL) - sitting it
    # exactly at the entry price would bury it inside the candle body,
    # since NEXT_CANDLE_OPEN's entry price IS that candle's open.
    ...
    # NEXT_CANDLE_OPEN is this strategy's only entry type - the marker alone
    # is enough for it; other entry types (unused here, kept for schema
    # parity with the sibling projects) still get their name spelled out.
    if entry.entry_type.value != "NEXT_CANDLE_OPEN":
        ax.annotate(
...
def render_trade_snapshot(result: TradeResult, candles_fine: pd.DataFrame) -> str | None:
    ...
    window_start = (result.structure.ts if result.structure else result.entry.entry_time) - pad
...
def render_live_chart(result: TradeResult, candles_fine: pd.DataFrame) -> bytes | None:
    ...
    shows just the pre-cutoff swing band on a NO_SETUP day, adds the
    structure marker once structure resolves, and the entry zone/SL/TP once
    an entry is found. Returns PNG bytes directly (not saved to disk - this
    is regenerated fresh on every request, not archived per-trade)."""
