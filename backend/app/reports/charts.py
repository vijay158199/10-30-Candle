...
_STRUCTURE_COLORS = {"SWING_BREAK": "#06b6d4"}
_ENTRY_COLORS = {
    "CISD": "#ec4899",
    "ORDER_BLOCK": "#3b82f6",
...
def render_trade_snapshot(result: TradeResult, candles_fine: pd.DataFrame) -> str | None:
    ...
    without cross-referencing the trade log row:
      - a horizontal line at the fractal swing high/low that was broken
    ...
    # 1) The swing level that was broken (this project's "liquidity" - the
    #    fractal swing high/low the breaking candle closed beyond)
    if result.structure is not None:
        ax.axhline(y=result.structure.broken_swing.price, color="#94a3b8", linestyle="-", linewidth=1.1, alpha=0.6, zorder=0)
        ax.text(
            0, result.structure.broken_swing.price, f" swing {result.structure.broken_swing.kind} ",
            fontsize=7, color="#64748b", va="bottom", ha="left",
        )

    # 2) Swing-break marker (labelled near the top of the chart
